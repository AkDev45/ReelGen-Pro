...
const ResultsGrid: React.FC<ResultsGridProps> = ({ results, isRestrictedMode = false, onUpgradeClick }) => {
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-12 gap-6 animate-in fade-in duration-700 slide-in-from-bottom-4">
      ...
      {/* Roast is usually visible, but maybe shortened in strict restricted mode, but here we show it fully or blurred? 
          Prompt said: Free 2/3 shows short roast. The current response has full roast. 
          We will simply show it because it's fun, or blur it if needed. 
          Let's leave Roast visible as "Surface Signal" but maybe just show it. 
      */}
      ...
      <div className="col-span-1 md:col-span-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-slate-800/60 backdrop-blur-md rounded-xl p-5 border border-slate-700/50 relative overflow-hidden">
               {isRestrictedMode && <LockedOverlay />}
               <div className={`${isRestrictedMode ? 'opacity-10 blur-sm' : ''}`}>
      ...
      {/* Script Section (Full width at bottom) */}
       <div className="col-span-1 md:col-span-12 bg-slate-800/60 backdrop-blur-md rounded-2xl p-6 border border-slate-700/50 relative overflow-hidden">
           {isRestrictedMode && <LockedOverlay />}
           <div className={`${isRestrictedMode ? 'opacity-10 blur-sm' : ''}`}>
               <div className="flex justify-between items-center mb-4">
