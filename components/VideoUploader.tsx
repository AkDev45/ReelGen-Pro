  ...
  return (
    <section className="bg-slate-800/60 backdrop-blur-md rounded-2xl p-6 shadow-xl border border-slate-700/50">
      ...
      <div className="flex justify-center mb-6">
        <div className="bg-slate-900/60 p-1 rounded-xl flex flex-wrap gap-1 border border-slate-700/50">
          <button
            onClick={() => setMode('analyze')}
            className={`px-4 md:px-6 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
              mode === 'analyze' 
          ...
          <button
            ...
            className={`px-4 md:px-6 py-2 rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 ${
              mode === 'remix' 
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-2/5 flex flex-col">
          ...
          <div 
            className="flex-1 border-2 border-dashed border-slate-700 hover:border-indigo-500/50 transition-colors p-6 rounded-xl bg-slate-900/40 flex flex-col items-center justify-center cursor-pointer group"
          ...
          <div className="mt-4">
            ...
            >
              ...
              {isProcessing 
                ...
                : (mode === 'remix' ? "Extract & Remix Scripts" : "Generate Magic")
              }
            </button>
          </div>
        </div>
        ...
        {/* Right: Preview Area */}
        <div className="w-full md:w-3/5 bg-slate-900/40 p-1 rounded-xl border border-slate-700/50">
          <div className="relative w-full h-0 pb-[100%] md:pb-[56.25%] bg-black rounded-lg overflow-hidden">
            {videoState.url ? (
              <video 
                ref={videoRef} 
                src={videoState.url} 
